...
- On **DC-01**, created a root folder `C:\_Shared` with subfolders for `HR`, `IT`, and `Sales`.
...
- **The Fix:** On **DC-01**, I added the `Users` group to the `_Shared` root folder with **Read & Execute** permissions, but set it to **"This folder only"** so they couldn't see into other departments.
...
- Attempted to open the **HR** folder as the Sales user.
